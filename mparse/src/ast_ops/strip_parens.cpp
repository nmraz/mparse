...
mparse::ast_node_ptr strip_paren_step(mparse::ast_node_ptr node) {
...
void strip_parens(mparse::ast_node_ptr& node) {
  ...
  node = strip_paren_step(std::move(node));
}
