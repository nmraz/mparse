...
struct cast_visitor : ast_visitor {
  void visit(T& node) override { result = &node; }
...
inline T* ast_node_cast(U* node) {
  return static_cast<T*>(
      impl::cast_helper<std::remove_cv_t<T>, U>::do_cast(node));
}

template <typename T, typename U>
inline T* ast_node_cast(const U* node) {
  static_assert(std::is_const_v<T>,
                "ast_node_cast cannot remove const qualification");
...
inline node_ptr<T> static_ast_node_ptr_cast(const std::shared_ptr<U>& node) {
  return std::static_pointer_cast<T>(node);
}

template <typename T, typename U,
          typename = std::enable_if_t<std::is_base_of_v<ast_node, U>>>
inline node_ptr<T> ast_node_ptr_cast(const std::shared_ptr<U>& node) {
