...
struct print_visitor : mparse::ast_visitor {
...
void print_visitor::visit(mparse::unary_node& node) {
  node.child()->apply_visitor(*this);
}

void print_visitor::visit(mparse::unary_op_node& node) {
  result = "("s + stringify_unary_op(node.type()) + result + ")";
}

void print_visitor::visit(mparse::binary_op_node& node) {
...
void print_visitor::visit(mparse::literal_node& node) {
